...
describe('shareLinkRequestSchema', () => {
  it('accepts any subset of section toggles (all optional) and an empty body', () => {
    expect(shareLinkRequestSchema.safeParse({ share_map: true, share_budget: false, share_edit: true }).success).toBe(
      true,
    );
  ...
  it('rejects a non-boolean toggle', () => {
    ...
    expect(shareLinkRequestSchema.safeParse({ share_edit: 'yes' }).success).toBe(false);
  });
});
